...
import { UPDATE_STUB } from '../queryData/mutation'
import { Link } from 'react-router-dom'
import styled from "styled-components"
import 'bootstrap/dist/css/bootstrap.min.css'
import { client } from '../container/app'
import { Mutation } from "react-apollo"
import { currentUser, StubList, DETAIL, GET_USER } from '../queryData/query'
...
export default class AddNew extends React.Component {
    ...
    render() {
        ...
        return (
            <Mutation mutation={UPDATE_STUB}
                update={
                    (cache, response) => {
                        const data = cache.readQuery({ query: StubList })
                        var list = data.stubList.filter(stub => stub._id !== this.props.match.params.id)
                        list.push(response.data.updateStub)
                        data.stubList = list
                        cache.writeQuery({
                            query: StubList,
                            data
